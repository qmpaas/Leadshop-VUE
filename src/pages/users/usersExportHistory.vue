      ...
      <el-table :data="list" lazy="" style="width: 100%">
        <el-table-column label="注册时间">
          <template slot-scope="scope">
            <div class="he-text">{{ scope.row.conditions.created_time_start | getTime }} ~</div>
            <div class="he-text">
        ...
        <el-table-column label="上次消费时间" prop="title">
          <template slot-scope="scope">
            <div class="he-text">{{ scope.row.conditions.last_buy_time_start | getTime }} ~</div>
            <div class="he-text">
        ...
        <el-table-column label="最后访问" prop="title">
          <template slot-scope="scope">
            <div class="he-text">{{ scope.row.conditions.last_visit_time_start | getTime }} ~</div>
            <div class="he-text">
        ...
        <el-table-column label="消费次数" prop="created_time">
          <template slot-scope="scope">
            <div class="he-text">
              {{ scope.row.conditions.buy_number_start }}~{{ scope.row.conditions.buy_number_end }}
            </div>
          </template>
        </el-table-column>
        <el-table-column label="用户来源" prop="created_time">
          <template slot-scope="scope">
            <div class="he-text">
              {{ scope.row.conditions.source === 0 ? '微信小程序' : scope.row.conditions.source === 1 ? 'H5' : '全部' }}
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" prop="title">
          <template slot-scope="scope">
            <div class="he-button">
              <el-button
                @click="export2Excel(scope.row.user_data.tHeader, scope.row.user_data.list, scope.row.created_time)"
                class="he-class__button-new he-class__button-first"
                type="text"
              >
                下载 </el-button
              ><span>|</span>
...
export default {
  ...
  methods: {
    ...
    deleteItem(e) {
      this.$heshop
        .export('delete', e.id, { include: 'user' })
        .then(() => {
    ...
    export2Excel(tHeader, list, create_time) {
      require.ensure([], () => {
        this.$h.Export2Excel.export_json_to_excel({
          header: tHeader,
          data: list,
          filename: '用户报表(' + this.$moment(new Date(create_time * 1000)).format('YYYYMMDDHHmmss') + ')'
        });
      });
    }
  },
