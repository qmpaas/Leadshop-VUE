<template>
  <div class="specification">
    <el-form-item label="商品规格" required>
      <div class="specification-attr" v-for="(item, index) in selectData" :key="index + '_' + Date.parse(new Date())">
        <Item
          :ref="'selectItem'"
          v-model="selectData[index]"
          :is_cover="!is_delete(index)"
          :size="is_size(index)"
          :is_delete="is_delete(index)"
...
export default {
  ...
  methods: {
    ...
    getTemp: function (obj) {
      let { result } = obj;
      if (result.param_data.length === 0) return;
      let data = result.param_data.map(function (item) {
        return {
          image: '',
          value: item.name
        };
      });
      if (this.selectData.length === 1 && this.selectData[0].name === '') {
        this.selectData = [
          {
            image_status: false,
      ...
      this.$nextTick(() => {
        this.$refs.selectItem[this.$refs.selectItem.length - 1].handleDataSync();
      });
    }
  }
};
