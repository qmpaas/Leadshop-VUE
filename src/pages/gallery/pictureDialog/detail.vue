<template>
  <div class="picture-selet">
    <div class="picture-selet__detail">
      <el-card :style="{ height: limit > 1 ? '404px' : '536px' }">
        ...
        <div class="picture-selet__detail-group">
          <el-scrollbar :style="{ height: limit > 1 ? '355px' : '485px' }">
          ...
          <ul v-if="photoList.length > 0" v-loading="loading">
            <li v-for="(item, key) in photoList" :key="key">
              <label class="picture-selet__content-label" @dblclick.stop="handleDblClick(item)">
                <input
                  class="picture-selet__content-input"
                  :type="inputType"
                  :value="item"
                  v-model="checkedData"
                  :disabled="checkedData.length == limit"
                  v-if="item.type > 0"
                />
          ...
          <div v-else class="picture-selet__content-empty">
            <div>
              <el-image
                fit="cover"
                style="width: 100%; height: 100%"
                src="http://qmxq.oss-cn-hangzhou.aliyuncs.com/gallery_bg.png"
              ></el-image>
              <p>该分组下暂无内容</p>
            </div>
          </div>
    ...
    <div class="picture-selet__footer" v-if="limit > 1">
      <ul>
        <li>
          <div class="picture-selet__footer-title">已选 图片</div>
        </li>
        <li v-for="(item, key) in checkedData" :key="key">
          <div class="picture-selet__footer-image">
            ...
            <i
              class="le-icon le-icon-cha2 picture-selet__footer-close"
              @click="
                () => {
                  checkedData.splice(key, 1);
                }
              "
            ></i>
          </div>
