...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        'consignee_info.address': [{ required: true, message: '请输入详细地址信息', trigger: 'blur' }],
        addressList: [{ required: true, message: '请选择收货地址', trigger: 'change' }],
        'consignee_info.name': [{ required: true, message: '请输入收件人姓名', trigger: 'blur' }],
        'consignee_info.mobile': [{ required: true, message: '请输入联系方式', trigger: 'blur' }]
      },
      district: []
    };
  },
    ...
    affirm(e) {
      this.$refs['ruleForm'].validate(valid => {
        if (valid) {
          ...
          this.$heshop
            .order('put', { behavior: 'consignee' }, data)
            .then(() => {
