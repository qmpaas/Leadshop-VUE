<template>
  <div>
    <el-row class="he-card" v-if="list.length > 0">
      <el-col :span="8">
        <div class="he-header">
          <el-checkbox @change="selectAll('province')" v-model="provinceAll" :indeterminate="provinceIndeterminate"
            >全选</el-checkbox
          >
        </div>
        <div class="he-body">
          <el-card style="height: 320px; overflow-y: auto; border: none" shadow="never">
            ...
            <template v-for="(item, index) in list">
              <div
                :id="'#' + item.id"
                :style="index === p_index ? 'background:#F3F5F7;' : ''"
                class="he-body__item el-row--flex is-align-middle is-justify-space-between"
                :key="item.id"
              ...
              >
                <div class="el-row--flex is-align-middle is-justify-end">
                  <el-checkbox
                    v-if="!radio"
                    @change="pickerChange(item, index, 'province')"
      ...
      <el-col :span="8">
        <div class="he-header">
          <el-checkbox @change="selectAll('city')" v-model="cityAll" :indeterminate="cityIndeterminate"
            >全选</el-checkbox
          >
        </div>
        <div class="he-body">
          <el-card
            style="height: 320px; overflow-y: auto; border: none"
            shadow="never"
          ...
          >
            <template v-for="(item, index) in list[p_index].list">
              <div
                :id="'#' + item.id"
                :key="item.id"
                :style="index === c_index ? 'background:#F3F5F7;' : ''"
                class="he-body__item el-row--flex is-align-middle is-justify-space-between"
              ...
              >
                <div>
                  <el-checkbox
                    v-if="!radio"
                    @change="pickerChange(item, index, 'city')"
                    v-model="item.checked"
                    :indeterminate="item.isIndeterminate"
                    :disabled="item.unchecked || all"
                  >
      ...
      <el-col :span="8">
        <div class="he-header">
          <el-checkbox @change="selectAll('district')" v-model="districtAll" :indeterminate="districtIndeterminate"
            >全选</el-checkbox
          >
        </div>
        <div class="he-body">
          ...
          >
            <template v-for="(item, index) in list[p_index].list[c_index].list">
              <div :id="'#' + item.id" :key="item.id" class="he-body__item" @click="d_index = index">
                <el-checkbox
                  v-if="!radio"
                  @change="pickerChange(item, index, 'district')"
                  v-model="item.checked"
                  :disabled="item.unchecked || all"
                >
                </el-checkbox>
...
export default {
  ...
  methods: {
    // 根据传入数据重新获取省市区列表
    newList() {
      let list1 = JSON.parse(JSON.stringify(this.defaultList));
      let list2 = this.value.select ? this.value.select : [];
      let list3 = this.value.unchecked.selectDistrict ? this.value.unchecked.selectDistrict : [];
      list1 = this.disCheck(list1, list2);

      let defaultList = this.checkList(list1, list2, list3);
    ...
    disCheck: function (list1, list2) {
      for (let i = 0; i < list1.length; i++) {
        let new_edit = [];
        for (let j = 0; j < list2.length; j++) {
          list1[i].notDis = false;
          if (list1[i].id === list2[j].id) {
            list1[i].notDis = true;
            if (typeof list2[j].list != 'undefined') {
              new_edit = list2[j].list;
            }
            break;
          }
        }
        if (typeof list1[i].list != 'undefined') {
          list1[i].list = this.disCheck(list1[i].list, new_edit);
        }
      }
      return list1;
    },
    // 判断哪些省市区已经选择
    checkList(defaultList, edit, formDetail) {
        ...
        for (let j = 0; j < formDetail.length; j++) {
          if (defaultList[i].id == formDetail[j].id) {
            ...
            if (typeof formDetail[j].list != 'undefined') {
              new_form = formDetail[j].list; //存在时,给子集列表赋值
            }
            break;
          }
        }
        if (typeof defaultList[i].list != 'undefined') {
          defaultList[i].list = this.checkList(defaultList[i].list, new_edit, new_form);
        }
      }
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (list[i].checked || list[i].isIndeterminate) {
          ...
          let newValue = {
            ...
            isIndeterminate: list[i].isIndeterminate
          };
