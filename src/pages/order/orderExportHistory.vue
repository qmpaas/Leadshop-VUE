      ...
      <el-table :data="list" lazy="" style="width: 100%">
        ...
        <el-table-column label="支付方式" prop="title">
          <template slot-scope="scope">
            <div class="he-text">
              {{
                scope.row.conditions.pay_type === ''
                  ? '全部'
                  : scope.row.conditions.pay_type === 'wechat'
                  ? '微信'
                  : scope.row.conditions.pay_type === 'aliapy'
                  ? '支付宝'
                  : ''
              }}
            </div>
            ...
            <div class="he-text">
              {{
                scope.row.conditions.source === ''
                  ...
                  ? '微信小程序'
                  : scope.row.conditions.source === 'wechat'
                  ? '公众号'
                  : ''
              }}
            </div>
...
export default {
  ...
  methods: {
    ...
    deleteItem(e) {
      this.$heshop
        .export('delete', e.id, { include: 'order' })
        .then(res => {
