...
export default {
  ...
  computed: {
    ...
    notConfig: function () {
      return this.qrcode && this.qrcode.weapp && !this.qrcode.weapp.image;
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    downLoad() {
      ...
      new html2canvas(document.getElementById(id), {
        ...
        downloadBase64File(base64, name);
      });
    ...
    downLoadQrcode() {
      ...
      if (this.active === 0) {
        downloadBase64File(this.qrcode.weapp.image, name);
      } else {
        downloadBase64File(this.qrcode.wechat.image, name);
      }
    }
  },
  mounted() {
